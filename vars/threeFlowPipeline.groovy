...
def call(body) {
    ...
    try {
        if (env.BRANCH_NAME.startsWith('PR-')) {
            ...
            echo 'Snapshot Flow'
            def artifact = builder.build(push: true, tag: "snapshot-$tag", verbose: false, scm: scm)
            deployer.deploy(artifact: artifact, environment: 'develop')
            promoter.promote(branch: 'candidate')
        } else if (env.BRANCH_NAME == 'candidate') {
            echo 'Candidate Flow'
            def artifact = builder.build(push: true, tag: "candidate-$tag", verbose: false, scm: scm)
            deployer.deploy(artifact: artifact, environment: 'staging')
            promoter.promote(branch: 'release')
        } else if (env.BRANCH_NAME == 'release') {
            echo 'Release Flow'
            def artifact = builder.build(push: true, tag: "release-$tag", verbose: false, scm: scm)
